...
def chatbot(user_input, weight, height, age, gender, activity_level, desired_weight_loss_kg, food_name):
    ...
    response = f"\n\U0001F4DD Health Calculations:\nBMI: {bmi}\nBMR: {bmr}\nTDEE: {tdee}\nIBW: {ibw}\nCalories to lose {desired_weight_loss_kg} kg: {calories_to_lose_weight} cal\n"

    # Display free nutritional information only
    if nutrients:
        response += "\n\U0001F4CA Nutritional Information (Free Tier):\n"
        for item in nutrients:
            response += (
                f"Food: {item.get('name', '-')}\n"
                f"Total Fat: {item.get('fat_total_g', '-')} g\n"
                f"Saturated Fat: {item.get('fat_saturated_g', '-')} g\n"
                f"Sodium: {item.get('sodium_mg', '-')} mg\n"
                f"Potassium: {item.get('potassium_mg', '-')} mg\n"
                f"Cholesterol: {item.get('cholesterol_mg', '-')} mg\n"
                f"Carbohydrates: {item.get('carbohydrates_total_g', '-')} g\n"
                f"Fiber: {item.get('fiber_g', '-')} g\n"
                f"Sugar: {item.get('sugar_g', '-')} g\n\n"
            )
    else:
...
gr.TabbedInterface([chatbot_interface,diet_interface,], [ "Health Bot","Diet Plan"]).launch()
